...
- **Enrichment:** for each matched concept, fetches its page at `reference/<source>/<id>.md` (leading id segment dropped when it equals the source) and parses front matter for `kind`, `fields` (`{name, type}`), `summary`. The narrative link comes from the DOC_INDEX entry's `doc_path`. (The page's `extracted_from` Go-source list is parsed but not surfaced — nothing downstream consumes it.)
- Output: `grounding.json` with `{concepts: [{id, type_name, kind, source, summary, fields, narrative_doc}], concepts_total_matched, llms_txt_url}`
...
- For Hub-only: the current `docs/api-gateway/release-notes.mdx` head (last ~150 lines) so the LLM knows whether the target version's section already exists and what GA bullets are already there
...
- A `release-notes.mdx` patch — **Hub-only**, only when `needs_release_note=yes`. Patches `docs/api-gateway/release-notes.mdx` (never `docs/api-management/release-notes.md`, which is a re-import shim). Reorganized to per-version (semver) sections in [traefik/hub-doc#953](https://github.com/traefik/hub-doc/pull/953) — no longer month-based; see `references/release-note-heuristics.md` for the full structure. The patch shape depends on classification:
  - **New feature, Early Access**: insert a `#### <Feature Name>` subsection under the **target version**'s `### What's New`, prefixed with a `:::warning Early Access\n...\n:::` admonition, followed by 1-3 paragraphs of description, ending with `For configuration details, see the [<Feature Name>](<relative-link>) documentation.`
  - **New feature, GA**: same shape but **without** the EA admonition
  - **Graduated to GA**: prepend a bullet to the **top** of the existing `#### Graduated to GA` list under the target version, in the form `- **<Feature name>** is now generally available. See the [<doc title>](<relative-link>) documentation.`
  - **Breaking change**: add or append to a `#### Breaking Changes` subsection (the file currently doesn't have one — the patch creates it under the target version and flags the engineer to review the section heading choice)
  - **Compatibility-matrix change** (component version bump): update or insert the `<Collapse title="Compatibility matrix">` table under the target version (no longer a `#### Compatibility Matrix` heading — that heading was replaced by the Collapse wrapper in #953)
  - There is no way to derive the target **version** from the PR (unlike the old target month, which was just the merge date) — `classify.py` no longer returns a target field; SKILL.md asks the engineer/tech writer directly ("Which Hub release is this for?") and uses that as `target_version`. If `## Gateway v<target_version>` doesn't exist yet, create it (with its `**<YYYY-MM-DD>**` date line) at the top of the post-header area, populated with `### What's New` and the appropriate subsection.
  - **Insertion order (newest on top):** the entry is the latest change, so it never lands at the bottom. A new `#### <Feature>` subsection becomes the first feature subsection under that version's `### What's New` (after `#### Graduated to GA` if present, above other features and above the compatibility-matrix `<Collapse>`); new version sections go at the very top of the post-header area (never inside the `## Earlier releases` archive). `#### Graduated to GA` stays first and the compatibility-matrix `<Collapse>` stays last within a version.
...
- Runs a lint auto-fix pass, fully automatic, no confirmation:
  - File permissions: clears stray executable bits on written files.
  - Hub: `yarn docs:markdown --fix` resolves what's mechanical, then `yarn docs:markdown` + `yarn docs:alex` (no `--fix`, alex has none) capture what's left. `yarn build` is skipped — too slow for a preview loop.
  - OSS: `mkdocs build --strict -d /tmp/.mkdocs-preview` — structural, nothing to auto-fix.
  - Re-stages any auto-fixed files so the diff below reflects the fixed state.
- Prints `git diff --stat` then `git diff` (post-fix)
- Never blocks on lint state. Whatever the fixer couldn't resolve is returned as `lint_unresolved` and rendered into a `manual_checks_md` block for the doc PR body's "Manual checks required" section — visible to the reviewer on GitHub, not a terminal prompt.
...
2. **Commit the staged doc edits** on the feature branch before pushing. `preview.py` stages the generated files but does not commit; without this the pushed branch equals base and the draft PR is empty. Refuses if the repo isn't on the doc branch or nothing is staged.
3. If fork exists: `git push <fork-remote> docs/<slug>:docs/<slug>`
4. If no fork: prompt `[f] create fork via 'gh repo fork traefik/hub-doc --remote=false' / [u] push branch to traefik/hub-doc / [a] abort`
5. Open draft PR: `gh pr create --repo traefik/hub-doc --base main --draft --title "docs: <feature title>" --body-file <generated-body>`
6. The PR body (`pr-body.md.tmpl`) includes:
   ...
   - "Manual checks required" (from `preview.py`'s `manual_checks_md`, appended in step 8) — omitted entirely when the lint auto-fixer left nothing unresolved
...
**Which version:** since release-notes.mdx moved to per-version (semver) sections ([traefik/hub-doc#953](https://github.com/traefik/hub-doc/pull/953)), there's no merge-date shortcut anymore — which Hub release a change ships in isn't derivable from the PR alone. `needs_release_note` no longer returns a target field; SKILL.md asks the engineer/tech writer directly when `verdict=yes` ("Which Hub release is this release note for?") and the answer becomes `target_version`. The entry goes under `## Gateway v<target_version>` (created at the top if absent), not under whatever the newest heading happens to be.
...
| Linter fails | Auto-fix what's mechanical; flag the rest in the PR body's "Manual checks required" section; never blocks push |
