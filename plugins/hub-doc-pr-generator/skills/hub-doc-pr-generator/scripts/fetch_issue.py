...
def build_bundle(ref: IssueRef, *, impl_repo: str) -> dict:
    ...
    # Native GitHub sub-issue relationship (parent + siblings) and any PR that
    # closes this issue or a sibling, cross-repo included -- this entry point
    # previously never queried this at all, so it always came back
    # `parent: null` / `siblings: []` / `related_prs: []` even when GitHub's
    # graph had all three populated (see fetch_pr.py's fetch_issue_graph,
    # already used by the impl-PR path; this is the same call, just also
    # wired into the issue-only path).
    graph = fetch_issue_graph(ref.repo, ref.number)
